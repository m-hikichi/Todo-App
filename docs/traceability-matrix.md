...
|--------|--------|------------|--------------|------------|----------------|-----------|
| FC-01-01-001 | SPEC-001 | FR-001 | cmd/server/api.go, internal/todo/store.go, web/index.html, web/app.js, web/styles.css | handleTodos, CreateTodo, createTodo | internal/todo/store_test.go, 手動確認（タイトル欄は入力例プレースホルダで表示し、文字数制限は入力欄のカウンタで確認できる。メモ欄は`メモ（任意）`ラベルで表示） | ✅ |
| FC-01-01-002 | SPEC-001 | FR-002 | cmd/server/api.go, internal/todo/store.go, web/index.html, web/app.js, web/styles.css | handleTodoByID, UpdateTodo, submitTodoForm, updateTodo, beginEditing, scrollTodoFormIntoView, focusTodoTitleField | internal/todo/store_test.go, 手動確認（一覧の編集ボタンでフォーム展開、編集エリアへスクロール、タイトル入力へフォーカスして保存。詳細表示のメモ見出しも`メモ`で表示） | ✅ |
...
| FC-01-02-001 | SPEC-001 | FR-005 | cmd/server/api.go, internal/todo/store.go, web/index.html, web/app.js, web/styles.css | handleTodos, ListTodos, renderTodoView, getVisibleTodos, getTodoHeadingText, renderTodoList, renderTodoRow, toggleTodoDetails | internal/todo/store_test.go, 手動確認（`すべて`タブで全件表示。デスクトップでは固定ヘッダーなしの行型一覧、モバイルでは縦積み表示。ステータスが`待ち`や`完了`でも列位置が揃い、Chevronが右端で揃い、行本体クリックまたはフォーカス後の`Enter`/`Space`で詳細開閉） | ✅ |
...
| FC-02-01-002 | SPEC-001 | FR-009 | cmd/server/api.go, internal/todo/store.go, web/index.html, web/app.js, web/styles.css | handleTodos, handleTodoByID, CreateTodo, UpdateTodo, validateTodoValues, applyStoredDateTime, combineDateAndTime, bindTimeFieldPicker, openTimePicker | internal/todo/store_test.go, 手動確認（作成/編集フォームで開始予定が`日付`/`時刻`のグループとして表示され、開始時刻入力枠クリックでタイムピッカーを開く） | ✅ |
| FC-02-01-001 | SPEC-001 | FR-010 | cmd/server/api.go, internal/todo/store.go, web/index.html, web/app.js, web/styles.css | handleTodos, handleTodoByID, CreateTodo, UpdateTodo, validateTodoValues, isValidStartDue, applyStoredDateTime, bindTimeFieldPicker, openTimePicker | internal/todo/store_test.go, 手動確認（作成/編集フォームで締切が`日付`/`時刻`のグループとして表示され、日本時間案内が控えめな補助テキストになり、締切時刻入力枠クリックでタイムピッカーを開く） | ✅ |
...
| FC-04-02-001 | SPEC-002 | FR-002 | Dockerfile, compose.yaml, README.md, cmd/server/main.go | main | 手動確認（docker compose up -d --build で起動。Docker の target arch に追従して実行イメージを生成） | ✅ |
