...
- [x] FR-006: ユーザーはTodo状態を`active`、`in_progress`、`waiting`、`completed`に変更できる。現行実装では一覧上の状態ボタン（表示名は日本語）から1クリックで変更する。
...
- [x] FR-012: ユーザーは事前作成したプロジェクト（0または1件）をTodoへ紐づけ・解除できる。現行実装ではTodo作成/編集フォームの主要項目側に検索対応の単一選択UIを配置し、未設定のまま保存できる。
- [x] FR-021: ユーザーはTodo作成フォーム近傍のUIからプロジェクトを事前作成・編集・削除し、以後のTodo作成・編集フォームで再利用できる。現行実装では`GET/POST/PATCH/DELETE /api/projects`で永続化し、`+ 新しいプロジェクト`のインライン作成と`プロジェクトを管理`ダイアログで操作する。
...
- Todo入力エリア: 作成フォームは折りたたみ可能とし、閉じた状態でも「新しいTodoを追加」の見出し、補助文、必要に応じて入力途中バッジと下書きサマリを表示する。開いた状態の主要構成はタイトル、説明、プロジェクト、締切日/締切時刻、操作ボタンで、追加モードでは`キャンセル/追加する`、編集モードでは`編集をキャンセル/更新する`を表示する。キャンセル操作では入力を破棄してフォームを折りたたむ。新規追加成功後はフォームを開いたまま維持し、詳細設定の開閉状態も維持する。編集更新成功後はフォームを折りたたむ。プロジェクト欄は検索付き単一選択UIとし、`未設定`を許容する。欄の近傍に`+ 新しいプロジェクト`と`プロジェクトを管理`導線を置き、インライン作成と管理ダイアログを開けるようにする。詳細設定（折りたたみ）には開始予定日/開始時刻、担当者、繰り返し、親タスクを配置する。日付入力欄はカレンダーアイコン付きで、クリック時にカレンダーポップオーバーを開いて日付選択し、表示形式は`YYYY/MM/DD`とする。日付欄のキーボード直接入力は許可しない
- Todo一覧エリア: 各項目に日本語表示の状態ボタン、編集、削除、担当者/プロジェクト/開始予定日/締め切り/繰り返し/親タスク表示を提供する。削除前には確認ダイアログを表示する。検索UI、状態フィルター、バックアップ導線は未実装
- プロジェクト管理ダイアログ: Todoフォーム近傍のボタンからモーダル表示し、プロジェクト名入力、保存ボタン、既存プロジェクト一覧、各プロジェクトの編集/削除ボタンを提供する
...
3. 必要に応じてTodoフォームの`+ 新しいプロジェクト`からその場でプロジェクトを作成するか、`プロジェクトを管理`からプロジェクトを作成・編集・削除する。
4. 作成・編集フォームでは既存プロジェクトを0または1件選択してTodoへ設定し、未設定のままでも保存できる。
...
| TC-015 | FR-012 | 事前作成したプロジェクトを1件選択してTodoへ紐づけ・解除できる | 正常系 |
...
| TC-032 | FR-021 | `+ 新しいプロジェクト`でプロジェクトを作成すると、その場で候補へ反映され自動選択される | 結合テスト |
...
| 2026-03-15 | プロジェクト操作の主導線はTodoフォーム近傍に置き、インライン作成と管理ダイアログを併用する | Todo入力中に文脈を崩さずにプロジェクトを作成・選択・管理できるようにするため |
