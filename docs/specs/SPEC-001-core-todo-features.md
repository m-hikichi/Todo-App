...
- [x] FR-006: ユーザーはTodo状態を`active`、`in_progress`、`waiting`、`completed`に変更できる。現行実装では一覧上の状態ボタン（表示ラベルは日本語）から1クリックで変更する。
...
- Todo入力エリア: 作成フォームは折りたたみ可能とし、閉じた状態でも「新しいTodoを追加」の見出し、補助文、必要に応じて入力途中バッジと下書きサマリを表示する。開いた状態の最小構成はタイトル、説明、締切日/締切時刻、操作ボタンで、追加モードでは`キャンセル/追加する`、編集モードでは`編集をキャンセル/更新する`を表示する。キャンセル操作では入力を破棄してフォームを折りたたむ。新規追加成功後はフォームを開いたまま維持し、詳細設定の開閉状態も維持する。編集更新成功後はフォームを折りたたむ。詳細設定（折りたたみ）に開始予定日/開始時刻、担当者、ラベル、繰り返し、親タスクを配置する。ラベルは作成済みラベルを複数選択できるチップ群のみを表示し、新規作成/編集/削除はヘッダーのラベル管理ダイアログで行う。日付入力欄はカレンダーアイコン付きで、クリック時にカレンダーポップオーバーを開いて日付選択し、表示形式は`YYYY/MM/DD`とする。日付欄のキーボード直接入力は許可しない
- Todo一覧エリア: 各項目に日本語表示の状態ボタン、編集、削除、担当者/ラベル/開始予定日/締め切り/繰り返し/親タスク表示を提供する。削除前には確認ダイアログを表示する。検索UI、状態フィルター、バックアップ導線は未実装
