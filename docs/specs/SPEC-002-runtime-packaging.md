...
|--------|------------|----------|----------------------|
| F-007 | FC-04-01-001 | sqlite永続化（全モード） | FR-003 |
| F-008 | FC-04-02-001 | DockerでブラウザUI配信 | FR-001, FR-002 |
| F-009 | FC-04-03-001 | ヘルスチェック提供 | FR-007 |
| F-010 | FC-04-04-001 | host/port/data path設定 | FR-008 |
| F-011 | FC-04-02-002 | デスクトップウィンドウ操作 | FR-001, FR-006 |
| F-012 | FC-04-05-001 | Windows配布形式 | FR-004 |
| F-013 | FC-04-05-002 | macOS配布形式 | FR-005 |
| F-014 | FC-04-06-001 | Docker/Desktop挙動一致 | FR-009 |

## 機能仕様（実行・配布）
- [ ] FR-001: アプリは`docker`または`desktop`の起動モードで開始できる。
- [ ] FR-002: Dockerイメージにアプリバイナリと必要な実行アーティファクトを含め、`/`でブラウザUIを配信できる。
...
- [ ] FR-007: Dockerモードでは`GET /healthz`を公開し、運用監視に利用できる。
